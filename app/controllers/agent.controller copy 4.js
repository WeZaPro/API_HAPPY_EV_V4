  ...
  try {
    ...
    if (taxiId) {
      try {
        ...
        const staff = await TaxiDriver.findOne({
        ...
        if (phone) {
          ...
          if (updateFields.PAID_back) {
            status_paid_taxi = updateFields.PAID_back;
          }
          if (updateFields.PAID_go) {
            status_paid_taxi = updateFields.PAID_go;
          }
          ...
          // console.log("staffDriver  ", staffDriver);
          // console.log("staffDriver staffDriver  ", staffDriver.taxi_lpr);
          console.log("staffDriver phone ", staffDriver.phone);
          console.log("staffDriver driver ", staffDriver.driver);
          console.log("staffDriver driver_id ", staffDriver.staffDriver_id);
          console.log("data Booking_ID ", updateFields.data.Booking_ID);
          console.log("data START ", updateFields.data.START);
          console.log("data DESTINATION ", updateFields.data.DESTINATION);
          console.log("data RETURN_back ", updateFields.data.RETURN_back);
          // console.log("data PRICE ", updateFields.data.PRICE);
          console.log("data price_send ", price_send);
          console.log("data trip_type ", updateFields.data.trip_type);
          console.log("data trip_date ", updateFields.data.trip_date);
          console.log("data taxi_lpr ", updateFields.data.taxi_lpr);
          console.log("data old paid ", updateFields.data.paid);
          // console.log("data PAID_go ", updateFields.data.PAID_back);
          // console.log("data updateFields PAID_go ", updateFields.PAID_go);
          //

          console.log("status_paid_taxi >>>> ", status_paid_taxi);

          // update to รายได้ของคนขับ taxi
          // เอา staffDriver phone + Booking_ID + trip_type(go/back)ไป find table  รายได้ของคนขับ taxi แล้ว update
          // status_paid_taxi (0/1)
          // ให้ check ว่ามีข้อมูลหรือไม่ ถ้าไม่ให้ save ถ้ามีให้ update

          // ถ้า RETURN_back =1 => PRICE/2
        }
        // console.log("phone ", phone);
      } catch (err) {
        console.error("❌ Error while looking up phone from taxi_id:", err);
      }
    }
    //===============
...
exports.lineToDriver = async (req, res) => {
  try {
    ...
    const get_line_user_id_go = driverGo?.line_user_id || null;
    const get_line_user_id_back = driverBack?.line_user_id || null;
